/**
 * EditarAccionScreen
 * Formulario para editar una acción existente.
 * La lógica está en hooks/useEditarAccionForm.ts
 * Comparte layout con CrearAccionScreen.
 */
    ...
    {Platform.OS === 'web' ? (
      <input
        ...
        style={{
          ...
          outline: 'none', cursor: 'pointer', height: 36, boxSizing: 'border-box',
        }}
      />
    ) : (
      <TextInput
...
// ── Screen ────────────────────────────────────────────────────────────────────
interface Props { proyectoId: string; componenteId: string; accionId: string; }

export const EditarAccionScreen: React.FC<Props> = ({ proyectoId, componenteId, accionId }) => {
  ...
  return (
    <AppShell scrollable={false} style={s.shell}>
      {/* Header — Bug 1 fix: mobile 2 filas, desktop 1 fila */}
        ...
        {isMobile ? (
          <>
            ...
            <View style={s.topBarRow2}>
              <Text style={s.pageTitle}>Editar Acción</Text>
              <Text style={s.pageSubtitle}>Modifica la información de esta acción</Text>
            </View>
          </>
        ) : (
          <>
        ...
        >
          <View style={[s.formColumns, isMobile && s.formColumnsMobile]}>
            ...
            <View style={s.column}>
              ...
              {/* Tipos de Evidencia */}
              <View style={s.card}>
                <SectionHeader icon="document-text" title="TIPOS DE EVIDENCIA OPERATIVA" subtitle="Define los nombres que el responsable podrá usar al cargar evidencias." />
              ...
              <View style={[s.card, { zIndex: 10, position: 'relative' }]}>
                ...
                <View style={s.selectorRow}>
                  <View style={s.selectorWrap}>
                    <SearchableSelect options={opcionesAsignables} selectedValue={selectedUserId} onSelect={setSelectedUserId} placeholder="Buscar miembro del proyecto..." />
                  </View>
                  <Button label="Agregar" onPress={handleAgregarResp} disabled={!selectedUserId} style={s.btnAgregar} />
                </View>
                {asignados.length > 0 && (
                  <View style={s.memberGrid}>
                    {asignados.map(a => {
                      const isResp = a.tipoAsignacion === 'responsable';
                      ...
                      return (
                        <View key={a.id} style={s.memberCard}>
                          <View style={s.memberAvatarContainer}>
                            {a.fotoUrl
                              ? <Image source={{ uri: a.fotoUrl }} style={s.memberAvatarBg} />
                            ...
                            <View style={s.nameOverlay}>
                              <Text style={s.nameOverlayText} numberOfLines={1}>{name}</Text>
                            </View>
                            <TouchableOpacity style={s.removeBadgeFloat} onPress={() => handleRetirarResp(a.id)} activeOpacity={0.8}>
              ...
              <View style={s.card}>
                ...
                {requisitos.map((req, idx) => (
                  <View key={idx} style={s.reqRow}>
                    ...
                    <TextInput style={s.reqDesc} value={req.descripcion ?? ''} onChangeText={v => updateReq(idx, { descripcion: v || null })} placeholder="Descripción opcional" placeholderTextColor={colors.textSecondary} />
                    ...
                    <View style={s.minMaxRow}>
                      ...
                      <View style={s.minMaxItem}>
                        ...
                        <TextInput style={s.numInput} value={req.max_archivos != null ? String(req.max_archivos) : ''} onChangeText={v => updateReq(idx, { max_archivos: v ? parseInt(v) : null })} keyboardType="numeric" placeholder="Sin límite" placeholderTextColor={colors.textSecondary} />
                      </View>
                    </View>
                  </View>
                ))}
...
// ── Estilos (igual que CrearAccionScreen) ─────────────────────────────────────
