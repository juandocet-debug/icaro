...
import { crearComponenteMetaUseCase } from '../../../shared/dependencies';
...
export const CrearComponenteModal: React.FC<Props> = ({ proyectoId, metaId, visible, onClose, onCreado }) => {
  ...
  const handleGuardar = async () => {
    ...
    try {
      await crearComponenteMetaUseCase.ejecutar(proyectoId, metaId, { nombre: nombre.trim(), descripcion: descripcion.trim() || undefined });
      reset();
      onCreado();
    } catch (e: any) {
      setError(e?.response?.data?.error || 'Error al crear el componente.');
    } finally {
      setSaving(false);
    }
  };
      ...
      <View style={styles.center}>
        <View style={styles.card}>
          <View style={styles.header}>
            <Text style={styles.titulo}>Nuevo Componente</Text>
